///
/// NitroMathOnLoad.cpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © 2025 Marc Rousavy @ Margelo
///

#ifndef BUILDING_NITROMATH_WITH_GENERATED_CMAKE_PROJECT
#error NitroMathOnLoad.cpp is not being built with the autogenerated CMakeLists.txt project. Is a different CMakeLists.txt building this?
#endif

#include "NitroMathOnLoad.hpp"

#include <jni.h>
#include <fbjni/fbjni.h>
#include <NitroModules/HybridObjectRegistry.hpp>

#include "JHybridCameraViewSpec.hpp"
#include "views/JHybridCameraViewStateUpdater.hpp"
#include "JHybridMathSpec.hpp"
#include <NitroModules/JNISharedPtr.hpp>
#include <NitroModules/DefaultConstructableObject.hpp>

namespace margelo::nitro::math {

int initialize(JavaVM* vm) {
  using namespace margelo::nitro;
  using namespace margelo::nitro::math;
  using namespace facebook;

  return facebook::jni::initialize(vm, [] {
    // Register native JNI methods
    margelo::nitro::math::JHybridCameraViewSpec::registerNatives();
    margelo::nitro::math::views::JHybridCameraViewStateUpdater::registerNatives();
    margelo::nitro::math::JHybridMathSpec::registerNatives();

    // Register Nitro Hybrid Objects
    HybridObjectRegistry::registerHybridObjectConstructor(
      "Math",
      []() -> std::shared_ptr<HybridObject> {
        static DefaultConstructableObject<JHybridMathSpec::javaobject> object("com/margelo/nitro/math/HybridMath");
        auto instance = object.create();
        auto globalRef = jni::make_global(instance);
        return JNISharedPtr::make_shared_from_jni<JHybridMathSpec>(globalRef);
      }
    );
    HybridObjectRegistry::registerHybridObjectConstructor(
      "CameraView",
      []() -> std::shared_ptr<HybridObject> {
        static DefaultConstructableObject<JHybridCameraViewSpec::javaobject> object("com/margelo/nitro/math/HybridCameraView");
        auto instance = object.create();
        auto globalRef = jni::make_global(instance);
        return JNISharedPtr::make_shared_from_jni<JHybridCameraViewSpec>(globalRef);
      }
    );
  });
}

} // namespace margelo::nitro::math
